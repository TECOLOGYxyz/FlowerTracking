...
Interestingly, setting the value to 10 produced the best results in all these cases, except for the combination returning the lowest number of mismatches for NYAA-04.

Increasing the value for running mean means that less weight is put on movements between each frame.

Maybe NYAA-04 flowers move more? Maybe longer stalk?

Coincidently, the flowers in NYAA-04 also flower longer on average than the other series. 


If the average euclidean distance that a flower moves is higher for NYAA-04 it may improve results to even out this movement by increasing the value for running mean.
...
These parameters are not independent. For example, the distance between a point in the current frame and a point in the previous frame may not be the same as that for a point in the current frame and the mean location of the previous ten points in a track. This underlines the relevance of search for an optimal combination based on ground truth data.



Pointe: Selvom vi kigger på mange komvinationer af parameterne, så falder kombinationerne hvor de bedste resultater produceres (laveste mismatches) indenfor forholdsvis smalle områder. Det tyder på at metoden med de givne parametre er generaliserbar og kan anvendes på andre kameraer med samme setting.




For our application, we find that a 

For the three series for which the tracking algorithm failed to produce perfect results, the range 
